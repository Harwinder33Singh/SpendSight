...
class PlaidService {
    ...
    private func authHeader() async throws -> String {
        guard let token = await AuthService.shared.accessToken else {
            throw PlaidError.notAuthenticated
        }
        return "Bearer \(token)"
    }
    ...
    func createLinkToken() async throws -> String {
        ...
        if let error = response.error { throw PlaidError.serverError(error) }
        guard let linkToken = response.linkToken else { throw PlaidError.missingLinkToken }
        return linkToken
    }
...
enum PlaidError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notAuthenticated:     return "Please sign in to connect your bank."
